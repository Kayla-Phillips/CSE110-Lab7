describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    const prodItemsData = await page.$$eval('product-item', (prodItems) => {
      ...
      return prodItems.map(item => {
        ...
        const title = shadowRoot.querySelector('.title')?.textContent || '';
        const price = shadowRoot.querySelector('.price')?.textContent || '';
        const image = shadowRoot.querySelector('img')?.src || '';
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const button = await prodItem.evaluateHandle((item) => item.shadowRoot.querySelector('button'));
    ...
    const valueText = await text.jsonValue();
    expect(valueText).toBe("Add to Cart");
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const item of prodItems) {
      await item.evaluate((e) => {
        ...
        if (button && button.innerText === 'Add to Cart') {
          button.click();
        }
      });
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let correctButtons = true;

    for (const item of prodItems) {
      const button = await item.evaluateHandle((e) => e.shadowRoot.querySelector('button'), item);
    ...
    const cartCount = await page.$eval('#cart-count', (e) => e.innerText);
    expect(correctButtons).toBe(true);
    expect(cartCount).toBe('20');
